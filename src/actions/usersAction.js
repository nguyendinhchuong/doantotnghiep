...
export const onLogIn = user => {
  return (dispatch, getState) => {
    ...
    axios
      ...
      .then(res => {
        if (res.data.code === -1) {
          Promise.resolve(localStorage.removeItem("user")).then(() => {
            let chirp = { message: `Sai mật khẩu`, isRight: 0 };
            dispatch(message.message(chirp));
            dispatch(logInError(res));
          });
        } else if (res.data.code === -3) {
          Promise.resolve(localStorage.removeItem("user")).then(() => {
            let chirp = { message: `Tài khoản chưa tồn tại`, isRight: 0 };
            dispatch(message.message(chirp));
            dispatch(logInError(res));
          });
      ...
      .catch(err => {
        let chirp = { message: `Đăng nhập thất bại`, isRight: 0 };
        dispatch(message.message(chirp));
...
export const onLoadUsers = () => {
  return (dispatch, getState) => {
    ...
    axios
      ...
      .catch(err => {
        let chirp = { message: `Tải người dùng thất bại`, isRight: 0 };
        dispatch(message.message(chirp));
